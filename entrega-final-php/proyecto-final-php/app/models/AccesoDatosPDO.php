...
class AccesoDatos {
   ...
   // Constructor privado  Patron singleton
    ...
    public function getClientes ($primero,$cuantos,$campo,$orden):array {
        $tuser = [];
        // Crea la sentencia preparada
       // echo "<h1> $primero : $cuantos  </h1>";
        $stmt_usuarios  = $this->dbh->prepare("select * from Clientes order by $campo $orden limit $primero,$cuantos");
    ...
    public function lastId () {
        $cli = false;
        $stmt_cli   = $this->dbh->prepare("select max(id) from Clientes");
        if ( $stmt_cli->execute() ){
             if ( $obj = $stmt_cli->fetch()){
                $cli= $obj[0];
            }
    ...
    public function nextUser ($id,$field) {
        $cli = false;
        $stmt_cli   = $this->dbh->prepare("select * from Clientes where $field > :id order by $field ASC limit 1");
        $stmt_cli->setFetchMode(PDO::FETCH_CLASS, 'Cliente');
        $stmt_cli->bindParam(':id', $id);
        if ( $stmt_cli->execute() ){
             if ( $obj = $stmt_cli->fetch()){
    ...
    public function previousUser ($id,$field) {
        $cli = false;
        $stmt_cli   = $this->dbh->prepare("select * from Clientes where $field < :id order by $field DESC limit 1");
        $stmt_cli->setFetchMode(PDO::FETCH_CLASS, 'Cliente');
        $stmt_cli->bindParam(':id', $id);
        if ( $stmt_cli->execute() ){
             if ( $obj = $stmt_cli->fetch()){
    ...
    public function uniqueEmail ($email) {
        $cli = false;
        $stmt_cli   = $this->dbh->prepare("SELECT count(*) as count FROM Clientes WHERE email=:email ");
        $stmt_cli->setFetchMode(PDO::FETCH_ASSOC);
    ...
    public function uniqueEmailExistingUser ($email,$user) {
        $cli = false;
        $stmt_cli   = $this->dbh->prepare("SELECT count(*) as count FROM Clientes WHERE email= :email  and id != :id");
        $stmt_cli->setFetchMode(PDO::FETCH_ASSOC);
        $stmt_cli->bindParam(':email', $email);
        $stmt_cli->bindParam(':id', $user);
        if ( $stmt_cli->execute() ){
             if ( $obj = $stmt_cli->fetch()){
                $cli= $obj['count'];
            }
    ...
    public function getUserData ($login) {
        $cli = false;
        $stmt_cli   = $this->dbh->prepare("SELECT password, role FROM Users WHERE login= :login5");
        $stmt_cli->setFetchMode(PDO::FETCH_ASSOC);
        $stmt_cli->bindParam(':login5', $login);
        if ( $stmt_cli->execute() ){
             if ( $obj = $stmt_cli->fetch()){
                $cli= $obj;
            }
